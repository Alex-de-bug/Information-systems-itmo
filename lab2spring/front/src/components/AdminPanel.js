...
const AdminPanel = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`http://${process.env.REACT_APP_SERVER}/api/admin/requests`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
            }
        });
  ...
  const handleRequest = async (username, rulling) => {
    try {
        const response = await axios.post(`http://${process.env.REACT_APP_SERVER}/api/admin/requests`, {username: username, rulling: rulling}, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
            }
        });
