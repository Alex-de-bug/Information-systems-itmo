...
const AdminPanel = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      setLoading(true);
      try {
        const response = await axios.get('http://195.58.48.101/api/admin/requests', {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
            }
        });
  ...
  const handleRequest = async (username, rulling) => {
    try {
        const response = await axios.post('http://195.58.48.101/api/admin/requests', {username: username, rulling: rulling}, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
            }
        });
